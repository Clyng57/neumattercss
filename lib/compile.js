...
rtnConfigPath = ()=> {
  let pathN = path.resolve(__dirname + './neucss/');
  if (pathN.includes("node_modules")) return "../../../../neumatter.config"; 
  else return "../neumatter.config";
}

const nC = rtnConfigPath();
const _neu = require(nC);
...
rtnVal = (custom, def)=> {
  let rtnValues = '';
  if (custom === undefined || custom === null || custom === "default") {
    rtnValues = def + ";"
  } else {
    rtnValues = custom + ";";
  }
...
rtnOpaqueColor = (custom, customOpaque, def, opacity)=> {
  if (custom === undefined || custom === null || custom === "default" && 
  customOpaque === undefined || customOpaque === null || customOpaque === "default") {
    return def + ";";
  } else if (customOpaque === undefined || customOpaque === null || customOpaque === "default" && 
  custom.includes("var")) {
    return custom.replace(")", opacity) + ";";
  } else {
    return customOpaque + ";";
  }
}
...
rtnCss = (custom, css)=> {
  if (custom === undefined || custom === null || custom === false) return "";
  else return css;
}

rtnPath = (custom, def)=> {
  if (custom === undefined) return def;
  else return custom;
}

getFile = (customConfig, fileName)=> {
...
let darkMode = rtnCss(_neu.config.darkMode, varsBaseDark.base);
...
let neumatterConfig = [
  ...
  root.focus,
  root.systemMargin, root.systemHeight, root.systemBorder, root.systemBorderD2,
  root.end,
  darkMode,
  resetcss, basecss,
...
fs.writeFile( `${exportPath}neumatter.css`, neumatter, err => {
  if (err) throw err;
  console.log(
    chalk.magentaBright.bold('neumatter.css______________________________________\n'),
