...
rtnVal = (custom, def)=> {
  let rtnValues = custom === undefined || custom === null || custom === '' ? def + ';' : custom + ';';
  return rtnValues;
}

rtnOpaqueColor = (custom, customOpaque, def, opacity)=> {
  if (custom === undefined || custom === null || custom === '' && 
  customOpaque === undefined || customOpaque === null || customOpaque === '') {
    return def + ';';
  } else if (customOpaque === undefined || customOpaque === null || customOpaque === '' && 
  custom.includes('var')) {
...
let configVarsDark = variablesBase === 'base' ? varsBaseDark.base : varsBaseDark.baseLight;
...
const root = {
  start: `\n:root {`,
  defaults: variablesBase === 'base' ? varsBase.base : varsBase.baseLight,
